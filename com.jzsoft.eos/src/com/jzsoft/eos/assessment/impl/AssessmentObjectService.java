...
public class AssessmentObjectService extends DASDaoSupport implements
	...
	/* （非 Javadoc）
	 * @see com.jzsoft.eos.assessment.IAssessmentObjectService#getAllAssessmentObjects(com.eos.das.entity.criteria.CriteriaType)
	 */
	public AssessmentObject[] getAllObjects() {
		IDASCriteria dasCriteria = getDASTemplate().createCriteria(AssessmentObject.QNAME);
		AssessmentObject[] results = getDASTemplate().queryEntitiesByCriteriaEntity(AssessmentObject.class,
				dasCriteria);
	...
	public AssessmentObject[] loadObjectTaskTreeData() {
		...
		for (int i = 0; i < results.length; i ++) {
			getDASTemplate().getRelationEntities(AssessmentTask.class, results[i], "tasks");
		}
