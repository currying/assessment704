...
<style type="text/css">
    ...
    </style> </head> <body> <!--Layout--> <div class="nui-splitter"
...
<div class="nui-toolbar"
	...
	class="nui-button" iconCls="icon-add" plain="true"></a> <!-- 新建 --> <a
...
<div title="center" region="center"> <div id="itemAndGroupTabs"
	...
	title="考核指标" url="AssessmentItem.jsp"></div> <div title="考核体系"
	url="AssessmentItem.jsp"></div> </div> </div> </div> <script type="text/javascript">
		nui.parse();
		...
		function onTaskTreePreLoad(event) {
			...
			for (var i = 0; i < event.data.length; i++) {
				...
				if (event.data[i].assessmentTasks != undefined) {
					for (var j = 0; j < event.data[i].assessmentTasks.length; j++) {
						task = new Object;
						task.id = event.data[i].assessmentTasks[j].id;
						task.type = "task";
						task.name = event.data[i].assessmentTasks[j].number + "-"
								+ event.data[i].assessmentTasks[j].title;
